...
#         outputs_recon = self.recon_embed(hs[-1].flatten(0,1)).view(hs[0].shape[0],self.num_queries,-1)
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]} #, 'pred_recons': outputs_recon
        if self.aux_loss:
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, samples, outputs, targets, indices, num_boxes, log=True):
        ...
        indices = [(torch.as_tensor([10], dtype=torch.int64), torch.as_tensor([0], dtype=torch.int64)) for i in range(len(targets))]
        ...
        loss_ce = F.cross_entropy(src_logits[:,-1,:], target_classes_o, self.empty_weight)
...
#         target_masks = target_masks.view(src_masks.shape)
    ...
    def forward(self, outputs, samples, targets):
        ...
        for loss in self.losses:
            losses.update(self.get_loss(loss, samples, outputs, targets, indices, num_boxes))
...
#         weight_dict.update(aux_weight_dict)
    ...
    losses = ['labels', 'boxes'] #, ,'recons', 'boxes', 'cardinality']
#     if args.masks:
#         losses += ["masks"]
